const LoginSystem = (function() {
    ...
    function showNotification(containerId, type, message) {
        ...
        icon.className = '';
        const typeMap = {
            error: { containerClass: 'bg-red-50 text-red-800 dark:bg-gray-800 dark:text-red-400', iconClass: 'fas fa-exclamation-circle me-3' },
            success: { containerClass: 'bg-green-50 text-green-800 dark:bg-gray-800 dark:text-green-400', iconClass: 'fas fa-check-circle me-3' },
            info: { containerClass: 'bg-blue-50 text-blue-800 dark:bg-gray-800 dark:text-blue-400', iconClass: 'fas fa-info-circle me-3' }
        };
    ...
    async function validateAndRefreshToken() {
        ...
        if (!accessToken || Date.now() > expires) {
            ...
            if (refreshToken) {
                try {
                    const response = await apiPostRequest({ action: "refresh-token", refreshToken: refreshToken });
                    if (response.success) {
    ...
    function renderAndShowDashboard(userData) {
        ...
        dashboardPage.innerHTML = `
            <div id="sidebar" class="fixed left-0 top-0 h-full bg-white dark:bg-gray-800 shadow-lg sidebar-transition z-20 w-64 flex flex-col"></div>
        ...
        setupStaticEventListeners: function() {
            ...
            if (loginForm) {
                loginForm.addEventListener("submit", async (e) => {
                    ...
                    if (!nik || !password) return showNotification('login-message', 'error', 'NIK dan Password wajib diisi');
                    if (nik.length !== 16 || !/^\d+$/.test(nik)) return showNotification('login-message', 'error', 'NIK harus 16 digit angka');
            ...
            if (otpForm) {
                otpForm.addEventListener("submit", async (e) => {
                    ...
                    if (otp.length !== 6) return showNotification('otp-message', 'error', 'Masukkan 6 digit kode OTP');
